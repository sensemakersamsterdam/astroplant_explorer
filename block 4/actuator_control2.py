...
pwm_red.start(0)
pwm_blue.start(0)
json_str = '{"red": [{"time": "06:00", "value": 80}, {"time": "22:00", "value": 20}],"blue": [{"time": "06:00", "value": 80}, {"time": "22:00", "value": 20}],"farred": [{"time": "06:00", "value": 80}, {"time": "22:00", "value": 20}]}'
...
def makeList(colour):
    if colour in recipe:
        ...
        clr_list = []
        for element in clr:
            time_obj = time.strptime(element['time'], '%H:%M') #interpret as timenotation
            time_min = time_obj[3]*60 +time_obj[4] # convert to minutes
            clr_list.append((time_min, element['value'])) #convert to list pair
